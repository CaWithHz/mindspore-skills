...
> **⚠️ 最高优先级：三位一体校验 (Trinity Verification)**  
> 严禁单一来源盲从。开发决策必须基于 **[仓库现状]、[接口定义]、[工程规范]** 的三方对齐。  
> 本 skill 的流程、模板、命名约定都可能因 MindSpore 版本迭代而过时，每一步都必须先看仓库里同目录、同类算子的真实写法。

- **辩证参照**：① 参考同目录相似算子，但禁止盲从。若源码与接口定义（头文件）或项目规范冲突，以定义与最新规范为准，并向用户提示源码中的潜在遗留问题。② **版本前瞻**：目录结构、注册宏、Infer API 等随版本快速变化；开发前先 grep 确认当前分支真实用法；生成骨架后优先对齐近 6 个月内合入的代码。
- **属性正交与层级隔离**：① API 各维度属性（必选/可选、类型、值域）严格独立，禁止跨维度推演。② **层级约束**：核心声明（签名/强约束）> 次级说明（备注/建议）。典型预防：绝不因文档备注有「建议值」就误判参数为「可选」，防止因「推荐」描述导致调用代码缺项。
- **确定性原则**：① **无证不推**：接口/依赖须有物理证据（grep/头文件），禁止臆造。② **疑点上报**：当三者（文档/头文件/源码）冲突时，停止自动推断，列出冲突点请求人工仲裁。
- **工程格式规范（PR 门禁合规）**：① 代码与脚本：遵循 Pylint/PEP8、Shellcheck 及仓库 Clang-format 标准。② 文档与元数据：遵循 Markdownlint 与 Rstlint；UTF-8、Tab 缩进、禁 Windows 路径、单行 ≤120。③ 合规性自检：产出须预先通过 Check_Scanoss（开源合规）及上述门禁。
...
- **用户/场景视角（Step 8 测试阶段必须）**：典型使用场景、业界论文或大模型中的调用方式与输入规格是否在用例中覆盖；当前接入要求为**对标 PTA 一致**，与 PTA 对标通过即视为满足该视角。其余步骤可不单独列出，因评审视角的 PTA 对标项已覆盖"与业界实现一致"。

实现视角与评审视角的验证结果均需在"验证闭环"模板中显式记录；Step 8 还须在验收/测试报告中记录用户/场景视角的结论（或明确说明已通过 PTA 对标满足）。
...
2. 根据 D2 的触发条件判断是否需要沉淀（同类问题 ≥2 次 / 连锁遗漏 / 根因在流程层面），
...
> 不是每次绕过都要沉淀——一次性的上下文特例不写入规则，避免噪音。
