...
| **AI 存在幻觉风险** | 可能编造不存在的接口、参数或错误理解算子行为（如将「必选+建议值」误判为「可选」），尤其在缺乏上下文时 | skill 已内置关键约束：三位一体校验、辩证参照（禁止盲从/版本前瞻）、属性正交与层级隔离（禁止跨维推演）、确定性原则（无证不推、疑点上报）。见 SKILL 关键约束与 `reference.md` §25.2 |
