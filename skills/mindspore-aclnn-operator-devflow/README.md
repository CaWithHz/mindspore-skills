...
| `reference.md` | **细节与规则** | 30 章技术细节与代码骨架；被 workflows 按 §X 引用，需要时再读 |
...
| **框架微观实现细节可能滞后** | `gen_ops.py`、注册宏、CMake、GeneralInfer API、BPROP 框架等核心模块已基于源码分析并写入 `reference.md`（§3-§7），但框架持续演进，具体写法仍以仓库最新代码为准 | 实现时对照同目录已有算子的最新代码；skill 已内置"仓库现状 > 文档描述"原则 |
| **AI 存在幻觉风险** | 可能编造不存在的接口、参数或错误理解算子行为（如将「必选+建议值」误判为「可选」），尤其在缺乏上下文时 | skill 已内置核心行为准则：三位一体校验、辩证参照（禁止盲从/版本前瞻）、属性正交与层级隔离（禁止跨维推演）、确定性原则（无证不推、疑点上报）。见 SKILL 核心行为准则与 `reference.md` §25.2 |
| **隐性知识依赖人工补充** | 组内口头惯例、评审偏好、已废弃的流程（如"算子自测表"已取消）无法从文档或代码中获取 | 让有经验的开发者审阅 `checklists.md`，标注过时/缺失/实际做法不同的条目 |
| **排障案例不够丰富** | 常见编译报错、`gen_ops.py` 报错、ST 失败的典型原因和排查方法覆盖不全 | 收集组内 Top 10 常见问题，补充到 `reference.md` 对应章节 |
| **缺少真实用户反馈** | 目前未经大规模实际使用验证 | 使用中遇到 skill 指引与实际不符时，直接告诉 AI 具体情况（哪个算子、卡在哪步、实际做法是什么），AI 会按 SKILL.md 中的反馈收集机制评估是否需要更新 skill、更新哪些文件 |
| **PTA/CANN 版本差异** | PTA op-plugin、CANN/ACLNN 接口在不同版本间有差异，skill 未针对特定版本做适配 | 使用前确认版本矩阵，遇到版本差异时以实际头文件/源码为准 |
| **不同 AI 模型效果有差异** | 不同模型（或同一模型的不同版本）在指令跟随精度、代码理解深度、验证自觉性上存在差异，可能导致跳步、简化、或遗漏验证 | 首次用新模型时，先拿一个简单算子（如路径 1 的逐元素算子）跑完整 workflow 验证效果；skill 的验证闭环模板和 checklist 逐项"是/否"判定机制就是为了兜底模型差异——无论模型强弱，每步都必须给出证据 |
